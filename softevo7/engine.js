...
(function () {
  ...
  function createPopulation(genomes) {
    ...
    // 継続改善モード: viewer から渡されたシードゲノムを1世代目に使用
    const seed = (!genomes && window._relearnSeedGenome) ? window._relearnSeedGenome : null;
    if (seed) window._relearnSeedGenome = null; // 初回のみ適用

    for (let i = 0; i < COF.population; i++) {
      let g = genomes ? genomes[i] : null;
      if (!g && seed) {
        // エリートはそのまま、残りは少し変異させたコピー
        g = i === 0 ? seed : mutateGenome(seed, 0.15, 0.25);
      }
      population.push(new SoftBody(startX + i * spacing, sy, blueprint, g));
    }
    focusedIndex = 0;
  }
  ...
  // Map<slot(1-10), { resolve, reject }>
  let pendingSaveSlots = new Map();

  function spawnRaceOpponents() {
    raceOpponents = [];
    if (raceOpponentDefs.length === 0) return;
    const spacing = 120, sy = getSpawnY();
    for (let i = 0; i < raceOpponentDefs.length; i++) {
      const def = raceOpponentDefs[i];
      try {
        const genome = {
          weights: def.genome.weights.map(w => new Float32Array(w)),
          biases:  def.genome.biases.map(b => new Float32Array(b)),
        };
        ...
        opp.raceGenome    = def.genome;
        opp.raceBlueprint = def.blueprint;
        raceOpponents.push(opp);
      } catch (e) {
        console.warn('[RaceMode] 対戦相手のスポーン失敗:', e);
      }
    }
  }
  ...
  function evolve() {
    ...
    // ── 世代末保存キューの実行 ──────────────────────────────────────────
    if (pendingSaveSlots.size > 0) {
      const bestBody = population[indices[0]];
      const g = bestBody.brain.getGenome();
      const pendingData = {
        genome: {
          weights: g.weights.map(w => Array.from(w)),
          biases:  g.biases.map(b => Array.from(b)),
        },
        score:      Math.round(bestFit),
        generation,
        blueprint: {
        ...
        cof: Object.assign({}, COF),
      };
      for (const [, handlers] of pendingSaveSlots) handlers.resolve(pendingData);
      pendingSaveSlots.clear();
    }
  ...
  // ─── Physics (Verlet) ─────────────────────────────
  function physicStep() {
    const allBodies = raceOpponents.length ? population.concat(raceOpponents) : population;
  ...
  function renderSimPhase(simTime) {
    ...
    for (const opp of raceOpponents) {
      drawRaceOpponent(opp, simTime);
    }

    ctx.restore();

    // ── Brain mode: muscle activity mini-bars ──
    if (brainMode && population.length > 0 && focusedIndex < population.length) {
      const body = population[focusedIndex];
      const w = canvas.width / devicePixelRatio, h = canvas.height / devicePixelRatio;
      ctx.save();
      ctx.setTransform(devicePixelRatio, 0, 0, devicePixelRatio, 0, 0);
      const barW = 3, barMaxH = Math.min(20, h * 0.25), barGap = 2;
      const totalW = body.muscles.length * (barW + barGap);
      const startX = w - totalW - 4;
      const baseY = h - 4;
      // Background
      ctx.fillStyle = 'rgba(4,6,10,0.6)';
      ctx.fillRect(startX - 3, baseY - barMaxH - 10, totalW + 6, barMaxH + 14);
      // Label
      ctx.fillStyle = 'rgba(52,211,153,0.6)';
      ctx.font = '5px -apple-system,sans-serif';
      ctx.textAlign = 'left'; ctx.textBaseline = 'bottom';
      ctx.fillText('筋肉活動', startX, baseY - barMaxH - 2);
      for (let mi = 0; mi < body.muscles.length; mi++) {
        const m = body.muscles[mi];
        const act = Math.abs(m.currentOutput || 0);
        const bh = act * barMaxH;
        const x = startX + mi * (barW + barGap);
        ctx.fillStyle = 'rgba(52,211,153,0.15)';
        ctx.fillRect(x, baseY - barMaxH, barW, barMaxH);
        const alpha = 0.4 + act * 0.6;
  ...
  // ─── Race Opponent Rendering ─────────────────────
  function drawRaceOpponent(body, simTime) {
    const nodes = body.nodes, nc = nodes.length;
    if (nc === 0) return;
    let cx = 0, cy = 0;
    for (const n of nodes) { cx += n.x; cy += n.y; }
    ...
    // Bones
    ctx.strokeStyle = 'rgba(251,191,36,0.30)'; ctx.lineWidth = 1;
    for (const b of body.bones) {
      const na = nodes[b.a], nb = nodes[b.b];
      ctx.beginPath(); ctx.moveTo(na.x, na.y); ctx.lineTo(nb.x, nb.y); ctx.stroke();
    }
    // Muscles
    ctx.strokeStyle = 'rgba(255,165,50,0.55)'; ctx.lineWidth = 1.5;
    ...
    const labelW = 96, labelH = 26, lx = cx - labelW / 2, ly = cy - 74;
    ctx.beginPath();
    ctx.moveTo(lx + 4, ly); ctx.lineTo(lx + labelW - 4, ly);
    ctx.quadraticCurveTo(lx + labelW, ly, lx + labelW, ly + 4);
    ctx.lineTo(lx + labelW, ly + labelH - 4);
    ctx.quadraticCurveTo(lx + labelW, ly + labelH, lx + labelW - 4, ly + labelH);
    ctx.lineTo(lx + 4, ly + labelH); ctx.quadraticCurveTo(lx, ly + labelH, lx, ly + labelH - 4);
    ctx.lineTo(lx, ly + 4); ctx.quadraticCurveTo(lx, ly, lx + 4, ly); ctx.fill();
    ctx.fillStyle = 'rgba(251,191,36,0.97)';
    ctx.font = 'bold 9px sans-serif'; ctx.textAlign = 'center';
    ctx.fillText(`👑 ${body.raceName}`, cx, cy - 62);
    ctx.fillStyle = 'rgba(255,220,100,0.78)'; ctx.font = '8px sans-serif';
    ctx.fillText(`現在:${fit}  ベスト:${body.raceScore}`, cx, cy - 52);
    ctx.restore();
  }
  ...
  function computeStrategyProfile(brain) {
    ...
    // Indices: 0=hvel, 1=vvel, 2=mus, 3=gnd, 4=rht
    const typeNames = ['水平反応型', '落下制御型', '自己フィードバック型', '接地感知型', 'リズム駆動型'];
    let strategyType = typeNames[dominantIdx];

    const top1 = sortedN[0], top2 = sortedN[1], top3 = sortedN[2];
    const all = normalized;

    // ── Combined & special types (priority order) ──
    // All-balanced
    const minN = Math.min(...normalized), maxN = Math.max(...normalized);
    if (minN > 0.55) {
      strategyType = '汎用協調型';
    }
    // Both velocities dominant → 全方位運動型
    else if (top1.i <= 1 && top2.i <= 1 && top2.v > 0.6) {
      strategyType = '全方位運動型';
    }
    // gnd + rht → 歩行パターン型
    else if ((top1.i === 3 && top2.i === 4 || top1.i === 4 && top2.i === 3) && top2.v > 0.55) {
    ...
    // gnd + velocity (hvel or vvel) → 接地推進型
    else if (top1.i === 3 && top2.i <= 1 && top2.v > 0.5) {
      strategyType = '接地推進型';
    }
    // gnd + mus → 接触フィードバック型
    else if ((top1.i === 3 && top2.i === 2 || top1.i === 2 && top2.i === 3) && top2.v > 0.55) {
      strategyType = '接触フィードバック型';
    }
    // rht + velocity → 共鳴推進型
    else if (top1.i === 4 && top2.i <= 1 && top2.v > 0.5) {
      strategyType = '共鳴推進型';
    }
    // rht + mus → 律動制御型
    else if ((top1.i === 4 && top2.i === 2 || top1.i === 2 && top2.i === 4) && top2.v > 0.55) {
      strategyType = '律動制御型';
    }
    // hvel very dominant, vvel low → 水平特化型
    else if (top1.i === 0 && top1.v > 0.85 && all[1] < 0.3) {
      strategyType = '水平特化型';
    }
    // vvel very dominant, hvel low → 垂直制御型
    else if (top1.i === 1 && top1.v > 0.85 && all[0] < 0.3) {
      strategyType = '垂直制御型';
    }
    // gnd very dominant → 接地依存型
    else if (top1.i === 3 && top1.v > 0.85) {
      strategyType = '接地依存型';
    }
    // Three-way balance (top3 > 0.6)
    else if (top3.v > 0.6) {
      strategyType = '三元協調型';
    }
    // Generic two-way (top2 > 0.65)
    else if (top2.v > 0.65) {
  ...
  function generateStrategyNarrative(profile, body) {
    ...
    const stDesc = {
      '全方位運動型': '水平・垂直の両速度変化を総合的に活用した万能移動',
      '歩行パターン型': '接地感知とリズムを組み合わせた周期的歩行を実現',
      '適応制御型': '筋肉状態を監視しつつ速度変化にリアルタイムに適応',
      '汎用協調型': '全カテゴリをバランスよく活用する洗練された万能戦略',
      '接地推進型': '足が地面に触れた瞬間を検知して推進力に変換する',
      '接触フィードバック型': '接地情報と筋肉フィードバックで精密な姿勢制御',
      '共鳴推進型': 'リズム波形と速度フィードバックの共鳴で効率的に推進',
      '律動制御型': 'リズム信号に筋肉状態を加味した精密な周期運動',
      '水平特化型': '左右の速度変化のみに反応する高度に専門化した戦略',
      '垂直制御型': '垂直方向の動きを精密に制御し跳躍や姿勢維持を得意とする',
      '接地依存型': '接地センサーへの強い依存を持ち着地精度が高い',
      '三元協調型': '三つの主要カテゴリが均衡して協調する高次戦略',
    };
    if (stDesc[strategyType]) lines.push(stDesc[strategyType]);
  ...
  function renderBrainAnalysis() {
    ...
    let neededH;
    if (aCompact) {
      // Fixed grid: use canvas container height directly (no dynamic sizing)
      neededH = 0; // will use full container height
    } else {
      neededH = 22 + radarH + narrativeH + heatmapH + importanceH + trendH + 20;
    }
    const canvasH = aCompact ? 0 : Math.max(neededH, 380);
    ...
    if (aCompact) {
      // ═══════════════════════════════════════════════════════════
      // COMPACT 2×2 FIXED GRID LAYOUT (stable, no content-dependent sizing)
      // ┌─────────────────────┬──────────────────────────┐
      // │ Radar + Heatmap     │ Narrative+Importance+Trend│
      // ├─────────────────────┼──────────────────────────┤
      // │ 行動指標            │ 重み構造分析              │
      // └─────────────────────┴──────────────────────────┘
      // ═══════════════════════════════════════════════════════════
      ...
      // ── Fixed grid zones (ratio-based, not content-dependent) ──
      const upperRatio = 0.70;
      const midY = Math.floor(y + (ch - y - 4) * upperRatio);
      const contentBottom = ch - 4;
      const upperH = midY - y;
      const lowerH = contentBottom - midY - 6;

      // ════════ LEFT UPPER: Radar + Heatmap ════════
      let ly = y;
      const adaptiveRadarH = Math.min(radarH, Math.floor(upperH * 0.55));

      // ── Radar chart (pentagon) ──
      const rcx = halfW / 2, rcy = ly + adaptiveRadarH / 2;
      const rMax = Math.min(adaptiveRadarH / 2 - 10, halfW / 2 - 20);
      ...
      for (let ci = 0; ci < CAT_DEFS.length; ci++) {
        ...
        ctx.fillText(CAT_DEFS[ci].emoji, cx2, ly + 2);
      }
      ly += 6;
      const maxAttr = Math.max(...muscleAttr.flat(), 0.001);
      const hmMaxRows = Math.floor((midY - ly - 2) / rowH2);
      const hmRows = Math.min(dispMuscles, Math.max(1, hmMaxRows));
      for (let mi = 0; mi < hmRows; mi++) {
        ctx.fillStyle = 'rgba(255,255,255,0.5)';
      ...
      // ════════ RIGHT UPPER: fixed sub-zones (bottom-up allocation) ════════
      const trendChartH = 28;
      const trendTotalH = strategyHistory.length > 1 ? (9 + trendChartH + 12) : 0;
      const impRowH = 10;
      const impTotalH = 10 + CAT_DEFS.length * impRowH + 2;
      // Allocate from bottom: trend → importance → narrative fills rest
      const trendStartY = midY - trendTotalH;
      const impStartY = trendStartY - impTotalH;
      const narAvailH = impStartY - y - 12;

      let ry = y;
      // ── Narrative (top, fills remaining space) ──
      ctx.fillStyle = 'rgba(251,191,36,0.7)';
      ...
      const maxNarLines = Math.max(1, Math.min(narrative.length, Math.floor(narAvailH / 10)));
      ...
      // ── Importance (fixed position above trend) ──
      ry = impStartY;
      ctx.fillStyle = 'rgba(251,191,36,0.7)';
      ...
      // ── Strategy trend (fixed at upper zone bottom) ──
      ...
      const metrics = [
        { label: '接地率', value: grndRate, color: '#63d2ff',
          judge: grndRate > 0.7 ? '安定' : grndRate > 0.3 ? '普通' : '浮遊' },
        { label: '運動効率', value: Math.min(1, efficiency / 200), color: '#fbbf24',
          judge: efficiency > 100 ? '高効率' : efficiency > 30 ? '普通' : '低効率' },
        { label: '対称性', value: symScore, color: '#34d399',
          judge: symScore > 0.8 ? '左右均等' : symScore > 0.5 ? '非対称' : '片側依存' },
      ...
      for (const m of metrics) {
        ...
        ctx.font = '5px -apple-system,sans-serif'; ctx.textAlign = 'left';
        ctx.fillText(m.judge, barX + mBarW + 2, bly + 4);
        bly += metricRowH;
      }
      ...
      const layerRowH = Math.min(18, Math.floor(lowerH / (nLayers + 0.5)));
      for (let l = 0; l < nLayers - 1 && bry + layerRowH <= contentBottom; l++) {
        ...
        const biasJudge = posRatio > 0.7 ? '興奮寄り' : posRatio < 0.3 ? '抑制寄り' : '均衡';
        ctx.fillText(`${layerLabel} ${biasJudge}`, colR, bry + 4);
        bry += 8;
  ...
  function switchToBuild() {
    ...
    // ビルドへ戻る際にキュー中の保存をキャンセル
    for (const [, handlers] of pendingSaveSlots) {
      handlers.reject(new Error('設計フェーズへ戻ったため保存をキャンセルしました'));
    }
    pendingSaveSlots.clear();
    clearPopulation(); raceOpponents = []; isPaused = false; simSpeed = 1;
  ...
  function updateBrainInfoOverlay(body) {
    ...
    // Speed: average velocity magnitude
    let avgSpd = 0;
    for (const n of body.nodes) {
      avgSpd += Math.sqrt(n.vx * n.vx + n.vy * n.vy);
    }
    avgSpd = (avgSpd / Math.max(1, body.nodes.length)).toFixed(1);
    left.innerHTML = `N:${body.nodes.length} M:${body.muscles.length}<br>接地:${grnd} 筋活:${muscAct}%<br>速度:${avgSpd}`;
    // Strategy info
    try {
      const profile = computeStrategyProfile(body.brain);
      right.innerHTML = `${profile.strategyType}<br>Fit:${fit}<br>世代:${generation}`;
    } catch(e) {
      right.innerHTML = `Fit:${fit}<br>世代:${generation}`;
    }
  }
  ...
  function mainLoop(timestamp) {
    ...
    if (!isPaused) {
      ...
      while (physicsAccum >= PHYSICS_DT && steps < maxSteps) {
        ...
        for (const body of raceOpponents) body.updateBrain(simTime);
        physicStep();
      ...
      for (const body of raceOpponents) calcFitnessForBody(body);
  ...
  // ── 再学習データを viewer.html から引き継ぐ ──────────
  let _relearnLoaded = false;
  (function applyRelearnData() {
    let raw;
    try { raw = localStorage.getItem('softevo7_relearn'); } catch (e) { return; }
    if (!raw) return;
    try {
      localStorage.removeItem('softevo7_relearn');
      const data = JSON.parse(raw);
      if (!data.blueprint?.nodes?.length) return;

      // blueprint をロード
      clearBlueprint();
      data.blueprint.nodes.forEach(n => blueprint.nodes.push({ x: n.x, y: n.y, radius: n.radius }));
      data.blueprint.bones.forEach(b => blueprint.bones.push({ a: b.a, b: b.b }));
      data.blueprint.muscles.forEach(m => blueprint.muscles.push({ a: m.a, b: m.b }));
      updateBuildInfo();
      _relearnLoaded = true;

      // 継続改善モード: genome を初期シードとして保持
      if (!data.fresh && data.genome) {
        window._relearnSeedGenome = {
      ...
      // フラッシュで通知
      const flash = document.getElementById('gen-flash');
      if (flash) {
        flash.textContent = data.fresh ? '🔄 ゼロから再学習モード' : '✨ 継続改善モード';
        flash.classList.add('show');
        setTimeout(() => flash.classList.remove('show'), 2000);
      }
    } catch (e) {
      console.warn('[SoftEvo] relearn data parse error:', e);
    }
  })();

  if (!_relearnLoaded) presetBlob(); // 通常起動時のみデフォルトプリセット
  ...
  window.SoftEvoAPI = {
    ...
    getBestCreatureData() {
      ...
      const g = best.brain.getGenome();
      return {
        genome: {
          weights: g.weights.map(w => Array.from(w)),
          biases:  g.biases.map(b => Array.from(b)),
        },
        score:      Math.round(best.fitness || (best.getCenterX() - best.startX)),
        generation,
        blueprint: {
          nodes:   blueprint.nodes.map(n => ({ x: n.x, y: n.y, radius: n.radius })),
          bones:   blueprint.bones.map(b => ({ a: b.a, b: b.b })),
    ...
    addRaceOpponent(def) {
      if (!def?.genome?.weights || !def?.genome?.biases) return;
      raceOpponentDefs.push({
        genome:    def.genome,
    ...
    getRaceStatus() {
      return raceOpponents.map(opp => ({
        name:      opp.raceName,
        score:     Math.round(opp.fitness || 0),
        baseScore: opp.raceScore,
      }));
    },

    getGeneration()  { return generation; },
    getBestFitness() { return bestEverFitness; },

    /** 世代終了時に1位の生物データを解決する Promise を返す */
    queueSaveToSlot(slot) {
      return new Promise((resolve, reject) => {
        pendingSaveSlots.set(slot, { resolve, reject });
      });
    },

    /** キュー中の保存をキャンセルする */
    cancelPendingSave(slot) {
      const handlers = pendingSaveSlots.get(slot);
      if (handlers) {
        handlers.reject(new Error('キャンセルされました'));
        pendingSaveSlots.delete(slot);
      }
    },

    /** 指定スロットが保存待ち中か調べる */
    hasPendingSave(slot) {
      return pendingSaveSlots.has(slot);
    },
  };
