...
function detectDraw(hole, community) {
  ...
  if (straightType === 'open')  { outs += 8; labels.push('OESD'); }
  else if (straightType)        { outs += 4; labels.push('ガットショット'); }
 ...
 */
function getPostflopRecommendation(analysis, facingBet, potOddsPct) {
  const { tier, equity } = analysis;

  if (!facingBet) {
    if (tier === 'monster' || tier === 'strong') return 'bet';
    if (tier === 'draw' && equity >= 35)          return 'bet'; // セミブラフ
    return 'check';
  }

  if (tier === 'monster' || tier === 'strong') return 'call';
  if (tier === 'medium')                        return potOddsPct <= 30 ? 'call' : 'fold';
  if (tier === 'draw')                          return equity >= potOddsPct ? 'call' : 'fold';
  return 'fold';
}
...
function buildPostflopGuide() {
  ...
  html += '<div class="guide-formula">推定勝率が必要勝率を<b>上回っていればコールはプラス収支</b>。'
        + '下回るならフォールドが正解。</div>';
