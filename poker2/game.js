...
function _givePostflopFb(action) {
  ...
  const opt       = getPostflopRecommendation(analysis, facingBet, potOdds);
  ...
  if (action === opt) {
    ...
    if (analysis.tier === 'draw') msg += ` (勝率${analysis.equity}% vs 必要${potOdds}%)`;
    fb(msg, 'ng');
  }
  _renderStats();
}
...
function _renderAnalysis() {
  ...
  let html = `<span class="tier-chip" style="background:${info.color}">${info.label}</span>`;
  html += `<span class="hand-detail">${analysis.label}</span>`;
  if (analysis.outs) {
    html += `<span class="equity-label">アウツ${analysis.outs}枚 → 勝率約${analysis.equity}%</span>`;
  }

  bar.innerHTML = html;
  bar.style.display = 'flex';

  // ポットオッズ (相手のベットに直面している場合のみ)
  if (oddsEl) {
    if (currentBet > 0 && (gs === GS.FLOP || gs === GS.TURN || gs === GS.RIVER)) {
      const req = calcPotOdds(currentBet, pot);
      oddsEl.textContent = `必要勝率: ${req}%　(コール${currentBet} / ポット${pot})`;
