...
// ===== 統計 =====
let preflopStats  = { n: 0, ok: 0 }; // プリフロップ判断の正解率
let postflopStats = { n: 0, ok: 0 }; // ポストフロップ判断の正解率
let lastEquity    = 0;               // 直前に表示した推定勝率 (_renderHandInfoでキャッシュ)
let fbTimer = null;
...
function pCall() {
  ...
  currentBet   = 0; // フロップ開始時点では誰もベットしていない
  gs           = GS.FLOP;
  _dealCommunity(3);
}
...
function pCheck() {
  ...
  if (currentBet > 0) return; // ベットに直面している場合は不可
  _givePostflopFb('check');
  _advanceStreet();
}
...
function pBet() {
  ...
  const betAmt = Math.min(playerStack, Math.max(BB, Math.floor(pot * 0.5)));
  playerStack -= betAmt;
  pot         += betAmt;

  // ディーラーの反応: 55% フォールド, 45% コール
  if (Math.random() < 0.55) {
    playerStack += pot;
    _endHand();
    showResult(`✅ ディーラーがフォールド！<br><span class="res-score">+<b>${pot}</b> チップ獲得</span>`);
  } else {
    pot += betAmt; // ディーラーのコール分
    _advanceStreet();
  }
}
...
function pCallBet() {
  if (gs === GS.IDLE || gs === GS.PREFLOP || gs === GS.SHOWDOWN) return;
  if (currentBet <= 0) return; // ベットに直面していない場合は不可
  _givePostflopFb('call');
...
function _dealerActs() {
  if (Math.random() < 0.48) {
    // チェック → プレイヤーは Check か Bet を選べる
    currentBet = 0;
    renderAll();
    _renderPostflopCtrl('noBet');
  } else {
    // ベット (ハーフポット) → プレイヤーは Fold か Call
    const betAmt = Math.max(BB, Math.floor(pot * 0.5));
    pot       += betAmt; // ディーラーのベットをポットへ
    currentBet = betAmt;
    renderAll();
    _renderPostflopCtrl('facingBet');
  }
}
...
/**
 * フロップ以降のアクションをベーシック戦略と比較してフィードバック
 * @param {'bet'|'check'|'call'|'fold'} action プレイヤーが選択したアクション
 */
function _givePostflopFb(action) {
  if (community.length < 3 || playerHand.length !== 2) return;

  const analysis  = analyzePostflopHand(playerHand, community);
  const facingBet = currentBet > 0;
  const potOdds   = facingBet ? calcPotOdds(currentBet, pot) : 0;
  const opt       = getPostflopRecommendation(analysis, facingBet, potOdds, lastEquity);
  ...
  if (action === opt) {
    ...
    let msg = `❌ 推奨: ${POSTFLOP_ACTION_LABELS[opt]}`;
    if (facingBet) msg += ` (勝率${lastEquity}% vs 必要${potOdds}%)`;
    fb(msg, 'ng');
  }
  _renderStats();
}
...
/**
 * 常時表示のハンド情報バーを更新する
 * - プリフロップ: 自分のホールカードの表記 (例: AKo)
 * - フロップ以降: 推定勝率・強さティア・役名 (+ドローならアウツ情報)
 * - ポットオッズ + 判定理由は #ctrl 内の _renderPotOdds() で別途更新
 */
function _renderHandInfo() {
  const bar = document.getElementById('hand-info-bar');
  if (!bar) return;

  if (gs === GS.IDLE || playerHand.length !== 2) {
    bar.innerHTML = '';
    lastEquity = 0;
    return;
  }
  ...
  if (community.length < 3) {
    // プリフロップ: 自分のハンド表記のみ表示 (勝率はまだ計算できない)
    bar.innerHTML =
      `<span class="hand-info-lbl">あなたの手:</span>` +
      `<span class="hand-info-key">${handKey}</span>`;
    lastEquity = 0;
    return;
  }

  const analysis = analyzePostflopHand(playerHand, community);
  const info     = TIER_INFO[analysis.tier];
...
/**
 * #ctrl 内のポットオッズ + 判定理由を更新 (相手のベットに直面している場合のみ表示)
 */
function _renderPotOdds() {
  const oddsEl = document.getElementById('pot-odds-disp');
  if (!oddsEl) return;

  if (currentBet > 0 && (gs === GS.FLOP || gs === GS.TURN || gs === GS.RIVER)) {
    const req  = calcPotOdds(currentBet, pot);
    const good = lastEquity >= req;
    const verdict = good
      ? `コール推奨 (勝率${lastEquity}% ≥ 必要${req}%)`
      : `フォールド推奨 (勝率${lastEquity}% < 必要${req}%)`;
    oddsEl.innerHTML =
      `必要勝率: ${req}%　(コール${currentBet} / ポット${pot})<br>` +
      `<span class="odds-verdict ${good ? 'good' : 'bad'}">${verdict}</span>`;
...
function _renderCards() {
  // プレイヤーの現在の最強ハンド (フロップ以降) — ハイライト判定に使用
  let pBest = null;
  ...
  if (commEl) {
    ...
    for (let i = 0; i < 5; i++) {
      const c = community[i];
      commEl.appendChild(c ? mkCard(c, pCards.includes(c)) : _emptySlot());
    }
  }

  // プレイヤーのホールカード (役を構成するカードを金色でハイライト)
  const plEl = document.getElementById('player-cards');
  if (plEl) {
    plEl.innerHTML = '';
    if (playerHand.length) playerHand.forEach(c => plEl.appendChild(mkCard(c, pCards.includes(c))));
  ...
  if (pnEl) {
    pnEl.textContent = pBest ? `[${pBest.name}]` : (handKey ? `[${handKey}]` : '');
  }
}
 ...
 * @param {boolean} highlight 役を構成するカードなら金色でハイライト
 */
function mkCard(card, highlight = false) {
  ...
  el.className = `card ${red ? 'red' : 'blk'}${highlight ? ' card-highlight' : ''}`;
...
function _renderPostflopCtrl(mode) {
  ...
  if (mode === 'noBet') {
    ...
    el.appendChild(_btn(`Bet ${betAmt}`, 'ab raise-btn', pBet));
  } else {
...
function _renderStats() {
  const fmt = s => s.n === 0 ? '—' : `${s.ok}/${s.n} (${Math.round(s.ok / s.n * 100)}%)`;
  const total = { n: preflopStats.n + postflopStats.n, ok: preflopStats.ok + postflopStats.ok };

  const elPre  = document.getElementById('sc-preflop');
  const elPost = document.getElementById('sc-postflop');
  const elTot  = document.getElementById('sc-total');
  const elTop  = document.getElementById('sc-total-top');

  if (elPre)  elPre.textContent  = fmt(preflopStats);
  if (elPost) elPost.textContent = fmt(postflopStats);
  if (elTot)  elTot.textContent  = fmt(total);
  if (elTop)  elTop.textContent  = fmt(total);
}
...
/**
 * リザルトオーバーレイを表示
 * @param {string}  html      表示するメッセージHTML
 * @param {boolean} showdown  trueの場合、両者のホールカードを小窓で表示する
 */
function showResult(html, showdown = false) {
  ...
  const sd = document.getElementById('res-showdown');
  if (sd) {
    if (showdown) {
      const pEl = document.getElementById('res-player-cards');
      const dEl = document.getElementById('res-dealer-cards');
      if (pEl) { pEl.innerHTML = ''; playerHand.forEach(c => pEl.appendChild(mkCard(c))); }
      if (dEl) { dEl.innerHTML = ''; dealerHand.forEach(c => dEl.appendChild(mkCard(c))); }
