...
let lastEquity    = 0;               // 直前に表示した推定勝率 (_renderAnalysisでキャッシュ)
let fbTimer = null;
...
/**
 * フロップ以降、現在のハンド強度・ドロー・ポットオッズを表示する
 */
/**
 * フロップ以降、現在のハンド強度・勝率・ポットオッズを表示する
 */
function _renderAnalysis() {
  ...
  const show = community.length >= 3 && playerHand.length === 2 && gs !== GS.IDLE;
