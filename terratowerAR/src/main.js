/**
 * main.js — Terra Tower AR bootstrap & game loop
 *
 * Key differences from the front-camera (terratower) version:
 *  - Rear camera: video is NOT CSS-mirrored, so pinch dot x is NOT inverted.
 *  - landmarkToWorld() in coords.js also uses un-inverted x for the same reason.
 */
...
function onPinchMove(nx, ny, handScale, handRoll) {
  ...
  // Apply hand roll to the grabbed object.
  // For the rear camera the physical tilt matches the on-screen appearance
  // directly (no mirror), so we apply -sin to get the correct rotation direction.
...
function updatePinchDot(nx, ny, visible, isIdle = false) {
  ...
  // Rear camera: video is NOT mirrored, so no x-inversion needed
  pinchDot.style.left = (nx * w) + 'px';
