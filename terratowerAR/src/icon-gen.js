/**
 * icon-gen.js — standalone script to generate Terra Tower AR PWA icons via Canvas API.
 *
...
export function drawIcon(ctx, size) {
  ...
  ctx.beginPath();
  ctx.arc(cx, ringCy, s * 0.3, 0, Math.PI * 2);
  ctx.stroke();
  ...
  // Draw stacked blocks inside the ring
  ...
  drawBlock(s * 0.36, ringCy - unit * 1.8, s * 0.28, unit * 0.9, 200); // top
  drawBlock(s * 0.32, ringCy - unit * 0.9, s * 0.36, unit * 0.9, 35);  // mid
  drawBlock(s * 0.28, ringCy,              s * 0.44, unit * 0.85, 270); // bottom

  // Corner brackets (AR reticle style)
  const br = s * 0.37;
  const bl = s * 0.06;
  ctx.strokeStyle = '#58a6ff';
  ...
  const bw = s * 0.4, bh = s * 0.16, bx = cx - bw / 2, by = s * 0.80;
  ctx.beginPath();
